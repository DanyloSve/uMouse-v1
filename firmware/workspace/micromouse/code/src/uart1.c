...
volatile uint8_t UART1_DMAbufferTX[6]; //6
volatile uint8_t UART1_DMAbufferRX[3];

volatile uint8_t UART1_DMAdataRXisReady;
...
{
	UART1_DMAdataRXisReady = 0;
	
	for (uint8_t i = 0; i < (sizeof(UART1_DMAbufferRX) / sizeof (uint8_t) ); ++i)
	{
		dest[i] = UART1_DMAbufferRX[i];
	}
}
